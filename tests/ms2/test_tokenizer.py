...
def test_tokenizer_training_is_deterministic_and_transcript_only(
    tmp_path: Path,
) -> None:
    ...
    corpus = json.loads(
        (
            paths.data_processed_ms2 / "ms2_tokenizer_training_corpus_v001.json"
...
def test_default_tokenizer_cache_is_scoped_by_repo_root(tmp_path: Path) -> None:
    ...
    assert (
        tokenizer_a.processor.serialized_model_proto()
        != tokenizer_b.processor.serialized_model_proto()
    )
