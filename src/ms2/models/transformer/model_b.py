...
class ModelB(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        self.output_bias = self.add_weight(
            name="model_b_output_bias", shape=(VOCAB_SIZE,), initializer="zeros"
        )
    ...
    def call(
        ...
        encoder_output, enc_attn, enc_mask = self.encoder(
            encoder_ids, training=training
        )
        decoder_output, _, cross_attn = self.decoder(
            decoder_ids, encoder_output, enc_mask, training=training, cache=cache
        )
        logits = tf.matmul(
            decoder_output, self.embedding.embeddings, transpose_b=True
        ) + tf.cast(self.output_bias, decoder_output.dtype)
        return {
            "logits": logits,
            "encoder_self_attn_weights": enc_attn,
