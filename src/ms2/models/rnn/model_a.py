...
class ModelA(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        self.branch3 = Branch3JointEncoder(
            self.shared_embedding,
            char_vocab_size=char_vocab_size,
            use_char_cnn=not plain_branch3,
            name="branch3",
        )
        ...
        self.decoder = FiLMLSTMDecoder(
            self.shared_embedding, use_film=not no_film, name="decoder"
        )
    ...
    def call(
        ...
        encoder_output, encoder_summary = self.refinement(
            fused, mask=context_mask, training=training
        )
        gamma, beta = self.film(encoder_summary)
        logits = self.decoder(
            decoder_inputs, encoder_summary, gamma, beta, training=training
        )
        return {
            "logits": logits,
            "gates": gates,
