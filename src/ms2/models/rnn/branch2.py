    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        self.gru1 = tf.keras.layers.Bidirectional(
            tf.keras.layers.GRU(D_B2, return_sequences=True), name="branch2_bigru_1"
        )
        self.dropout_rnn = tf.keras.layers.Dropout(dropout_rnn)
        self.gru2 = tf.keras.layers.Bidirectional(
            tf.keras.layers.GRU(D_B2, return_sequences=True), name="branch2_bigru_2"
        )

    def call(
        self, context_ids: tf.Tensor, training: bool = False
