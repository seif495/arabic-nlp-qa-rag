...
class SharedTokenEmbedding(tf.keras.layers.Layer):
    ...
    def __init__(
        self, vocab_size: int = VOCAB_SIZE, d_tok: int = D_TOK, **kwargs: object
    ) -> None:
    ...
    def compute_mask(
        self, token_ids: tf.Tensor, mask: tf.Tensor | None = None
    ) -> tf.Tensor:
        del mask
...
class CharacterEmbedding(tf.keras.layers.Layer):
    ...
    def __init__(
        self, vocab_size: int = 192, d_char: int = D_CHAR, **kwargs: object
    ) -> None:
        super().__init__(**kwargs)
        self.embedding = tf.keras.layers.Embedding(
            vocab_size, d_char, mask_zero=True, name="char_embedding"
        )
