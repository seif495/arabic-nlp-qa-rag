...
class GatedMerge(tf.keras.layers.Layer):
    ...
    def __init__(self, dropout_ff: float = 0.3, mean_merge: bool = False, **kwargs: object) -> None:
        ...
        self.gate_hidden = tf.keras.layers.Dense(D_FUSE, activation=tf.keras.activations.gelu, name="merge_gate_hidden")
    ...
    def call(self, branches: tuple[tf.Tensor, tf.Tensor, tf.Tensor], training: bool = False) -> tuple[tf.Tensor, tf.Tensor]:
        h1 = self.proj1(branches[0])
        h2 = self.proj2(branches[1])
        h3 = self.proj3(branches[2])
        if self.mean_merge:
            gates = tf.ones((*tf.shape(h1)[:-1], 3), dtype=h1.dtype) / tf.cast(3.0, h1.dtype)
        else:
