...
class Branch3JointEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, token_embedding: SharedTokenEmbedding, char_vocab_size: int = 192, dropout_emb: float = 0.2, use_char_cnn: bool = True, **kwargs: object) -> None:
        ...
        self.encoder = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(D_B3, return_sequences=True), name="branch3_bilstm")
        self.plain_projection = tf.keras.layers.Dense(192, name="plain_branch3_projection")

    def call(self, joint_ids: tf.Tensor, char_ids: tf.Tensor, training: bool = False) -> tuple[tf.Tensor, tf.Tensor]:
