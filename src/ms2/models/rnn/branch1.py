...
class StructuredPooling(tf.keras.layers.Layer):
    ...
    def call(self, sequence: tf.Tensor, mask: tf.Tensor | None = None) -> tf.Tensor:
        ...
        if mask is not None:
            scores = tf.where(tf.cast(mask, tf.bool), scores, tf.fill(tf.shape(scores), tf.constant(-1e9, scores.dtype)))
...
class Branch1QuestionEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, embedding: SharedTokenEmbedding, dropout_emb: float = 0.2, **kwargs: object) -> None:
        super().__init__(**kwargs)
        self.embedding = embedding
        self.dropout = tf.keras.layers.Dropout(dropout_emb)
        self.encoder = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(D_B1, return_sequences=True), name="branch1_bilstm")
        self.pool = StructuredPooling(name="branch1_structured_pool")

    def call(self, question_ids: tf.Tensor, training: bool = False) -> tuple[tf.Tensor, tf.Tensor]:
