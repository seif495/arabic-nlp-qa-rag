...
def question_answer_distance(context_tokens: list[str], question_tokens: list[str], answer_start: int) -> int:
...
def write_long_dependency_placeholder(path: Path) -> Path:
    ...
    path.write_text(json.dumps({"status": "missing_trained_checkpoint", "bins": []}, indent=2) + "\n", encoding="utf-8")
    return path
