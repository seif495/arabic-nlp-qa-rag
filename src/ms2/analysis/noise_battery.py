...
NOISE_TYPES = ("char_swap", "char_delete", "char_insert", "token_mask", "diacritic_inject")
...
def apply_noise(text: str, noise_type: str, rate: float, seed: int = 13) -> str:
    ...
    if noise_type == "token_mask":
        return " ".join("<unk>" if rng.random() < rate else token for token in text.split())
...
def write_noise_battery_placeholder(path: Path) -> Path:
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=("model", "seed", "noise_type", "rate", "token_f1", "status"))
        writer.writeheader()
        for model in ("A", "B"):
            for seed in (13, 42, 91):
                for noise_type in NOISE_TYPES:
                    for rate in NOISE_RATES:
                        writer.writerow({"model": model, "seed": seed, "noise_type": noise_type, "rate": rate, "token_f1": "", "status": "missing_trained_checkpoint"})
    return path
