...
def compute_text_stats(texts: List[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "tokens": get_stats(token_lengths)
    }
...
def run_corpus_analysis(paths=None) -> None:
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        for key in ["transcripts", "questions", "answers"]:
            ts = stats[key]["tokens"]
            f.write(f"| {key.capitalize()} | {ts['count']} | {ts['min']} | {ts['max']} | {ts['mean']:.1f} | {ts['median']} | {ts['std_dev']:.1f} |\n")
        ...
        for key in ["transcripts", "questions", "answers"]:
            ...
            f.write(f"| {key.capitalize()} | {cs['count']} | {cs['min']} | {cs['max']} | {cs['mean']:.1f} | {cs['median']} | {cs['std_dev']:.1f} |\n")
        ...
        f.write("*(TODO: Analyze the above stats and write 3-5 bullets regarding outliers, balance, length shifts, etc.)*\n")
        f.write("- **Outliers**: \n")
