...
def validate_pairing(paths: Optional[MS1Paths] = None) -> dict:
    ...
    if missing_qa:
        errors.append(
            f"Transcripts with no matching QA video_title: {sorted(missing_qa)}"
        )
    if missing_transcripts:
        errors.append(
            f"QA video_titles with no matching transcript: {sorted(missing_transcripts)}"
        )
...
def validate_records(qa_records: List[dict]) -> None:
    ...
    if errors:
        raise ValueError("QA record validation failed:\n" + "\n".join(errors))
